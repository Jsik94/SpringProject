...
<div class="container">
	...
	<div class="row">
		...
		<c:if test="${empty listPagingData.lists}" var="res">
				<tr>
		...
		<c:if test="${not res }">
			...
			<c:forEach var="item" items="${ listPagingData.lists}" varStatus="loop">
				<tr>
					<td>${listPagingData.pageSize - loop.index }</td>
					
					
					<td class="text-left"><a href="<c:url value="View.do?nowPage="/><c:out value="${param.nowPage }" default="1"/>&no=${item.no}">${item.title }</a> <span
