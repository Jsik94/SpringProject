		...
		<form action="<c:url value='/Validation/ValidationCheck.do'/>"
			...
			<table class="table table-bordered" bgcolor="gray" cellspacing="1"
				...
				<tr bgcolor="white">
					...
					<td><input type="text" name="name" value="${param.name}" /> <!-- 에러 메시지 표시 <접두어:errors path="커멘트드객체명.속성명"/>
				...
				<tr bgcolor="white">
					...
					<td><input type="checkbox" name="inters" value="정치"
						<c:if test='${fn:indexOf(requestScope.cmd.inters,"정") !=-1 }'>checked</c:if> />정치
				...
				<tr bgcolor="white">
					<td>자기소개</td>
					<td><textarea name="self" cols="30" rows="10">${self}</textarea>
